...
CREATE OR REPLACE FUNCTION public.portal_api_lock_automation_placement_target()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  PERFORM pg_advisory_xact_lock(hashtext('portal_api_automation:' || NEW.automation_id));
  IF EXISTS (
    SELECT 1
    FROM public.automatiseringen
    ...
    RAISE EXCEPTION 'Cannot place archived automation %', NEW.automation_id
      USING ERRCODE = '23514';
  END IF;
  RETURN NEW;
END;
...
REVOKE ALL ON FUNCTION public.portal_api_lock_automation_placement_target() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.portal_api_lock_automation_placement_target() FROM anon;
REVOKE ALL ON FUNCTION public.portal_api_lock_automation_placement_target() FROM authenticated;

DROP TRIGGER IF EXISTS portal_api_lock_automation_placement_target
  ON public.automation_placements;

CREATE TRIGGER portal_api_lock_automation_placement_target
  BEFORE INSERT OR UPDATE OF automation_id ON public.automation_placements
  FOR EACH ROW
  EXECUTE FUNCTION public.portal_api_lock_automation_placement_target();
