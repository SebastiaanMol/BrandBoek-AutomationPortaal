...
import { formatSyncPreviewImportedToast } from "@/lib/syncReviewToast";
...
// ── Static style constants (module-level to avoid recreation per render) ───────
...
function PortaalInstellingenCard(): React.ReactNode {
  ...
  function toggleStatus(s: typeof STATUSSEN[number]): void {
    const next = toggleInArray(local.beschikbareStatussen, s);
    if (next.length === 0) return; // at least one status must remain active
  ...
  function toggleCategorie(c: typeof CATEGORIEEN[number]): void {
    const next = toggleInArray(local.beschikbareCategorieen, c);
    if (next.length === 0) return;
    setLocal({ ...local, beschikbareCategorieen: next });
  }

  function toggleVerplicht(v: VerplichtVeld): void {
    setLocal({ ...local, verplichtVelden: toggleInArray(local.verplichtVelden, v) });
  }

  function addSysteem(): void {
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-6 space-y-5">
      ...
      {/* BEDRIJFSREGELS */}
      <div>
        <p className={LABEL_CLASS}>Bedrijfsregels</p>

        <div className={ROW_CLASS}>
          <span className={FIELD_LABEL_CLASS}>Verificatieperiode</span>
        ...
        <div className={ROW_CLASS}>
          <span className={FIELD_LABEL_CLASS}>Actieve statussen</span>
        ...
        <div className={ROW_CLASS}>
          <span className={FIELD_LABEL_CLASS}>Actieve categorieën</span>
          <div className="flex flex-wrap gap-2">
            {CATEGORIEEN.map((c) => (
      ...
      <div>
        <p className={LABEL_CLASS}>Weergave-standaarden</p>

        <div className={ROW_CLASS}>
          <span className={FIELD_LABEL_CLASS}>Standaard statusfilter</span>
          <select
        ...
        <div className={ROW_CLASS}>
          <span className={FIELD_LABEL_CLASS}>Standaard sortering</span>
          <select
      ...
      {/* DATAVELDEN */}
      <div>
        <p className={LABEL_CLASS}>Datavelden</p>

        <div className={ROW_CLASS}>
          <span className={FIELD_LABEL_CLASS}>Verplichte velden</span>
          <div className="flex flex-wrap gap-2">
        ...
        <div className={ROW_CLASS}>
          <span className={FIELD_LABEL_CLASS}>Extra systemen</span>
        ...
        <div className={ROW_CLASS}>
          <span className={FIELD_LABEL_CLASS}>Extra categorieën</span>
          <div className="space-y-2">
            <div className="flex gap-2">
              <input
                type="text"
...
function IntegrationCard({
  ...
  async function handleConnect(rawToken: string): Promise<void> {
    try {
      await saveIntegration.mutateAsync({ type, token: rawToken });
      setToken("");
      toast.success(`${label} verbonden`);
    } catch (e: unknown) {
      toast.error(e instanceof Error ? e.message : "Verbinding mislukt");
    }
  }
  ...
  async function handleSync(): Promise<void> {
    try {
      ...
      if (isReviewableSyncSource(type) && hasReviewItems(result)) {
        toast.info(formatSyncPreviewImportedToast(label, result.changeItems));
        return;
      }
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-6 space-y-5">
      ...
      {!isLoading && !isConnected && (
        renderConnectForm
          ? renderConnectForm({ onConnect: handleConnect, isPending: saveIntegration.isPending })
          : (
            <div className="space-y-3">
              <div className="space-y-1.5">
                ...
                {/* Raw HTML intentional — content is a hardcoded string prop, never user-supplied */}
                <p className="text-xs text-muted-foreground" dangerouslySetInnerHTML={{ __html: tokenHint }} />
              </div>
              <button onClick={() => handleConnect(token.trim())} disabled={saveIntegration.isPending || !token.trim()}
                className="flex items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50 transition-colors">
  ...
  async function handleFileChange(event: React.ChangeEvent<HTMLInputElement>): Promise<void> {
    ...
    try {
      ...
      if (hasReviewItems(result)) {
        toast.info(formatSyncPreviewImportedToast("Zapier", result.changeItems));
        return;
      }
...
function GitLabConnectForm({
  ...
  async function handleConnect(): Promise<void> {
    ...
    await onConnect(JSON.stringify({ pat: pat.trim(), projectId: projectId.trim(), branch: branch.trim() || "main" }));
    setPat("");
    setProjectId("");
    setBranch("main");
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <button
        ...
        disabled={isPending || !pat.trim() || !projectId.trim()}
        className="flex items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50 transition-colors"
      >
        <Link2 className="h-3.5 w-3.5" />
        {isPending ? "Verbinden..." : "Verbinden"}
      </button>
    </div>
  );
}
...
export default function Instellingen(): React.ReactNode {
  ...
  async function handlePipelinesSync(): Promise<void> {
    try {
    ...
    },
    {
      ...
      node: (
        <IntegrationCard
          type="gitlab"
          label="GitLab"
          description="Lees automation-bestanden en genereer AI-beschrijvingen"
          badge="GL"
          badgeClass="bg-orange-50 border border-orange-100 text-orange-600"
          tokenLabel="Personal Access Token"
          tokenPlaceholder="glpat-xxxxxxxxxxxxxxxxxxxx"
          tokenHint=""
          syncMutation={gitlabSync}
          renderConnectForm={(props) => <GitLabConnectForm {...props} />}
        />
      ),
    },
